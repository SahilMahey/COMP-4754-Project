...
function MovieModal({ movie, setIsModalOpen }) {
  const { poster_path, title, overview, genres, vote_average, release_date, runtime, status, type, popularity, production_companies } = movie;
        ...
        <div className="flex flex-col md:flex-row gap-8">
          {poster_path && (
            <div className="flex-shrink-0">
              <img src={poster_path} alt={title} className="w-64 rounded-lg shadow-xl hover:scale-105 transition-transform duration-300" />
            </div>
          )}
          ...
          <div className="flex-grow">
            ...
            <div className="space-y-4">
              <p className="text-gray-300">
                <span className="font-semibold text-red-400">Overview:</span> {overview}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Genre:</span> {genres.join(", ")}
                </p>
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Rating:</span> {vote_average}
                </p>
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Release Date:</span> {new Date(release_date).toLocaleDateString()}
                </p>
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Runtime:</span> {runtime} minutes
                </p>
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Status:</span> {status}
                </p>
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Type:</span> {type}
                </p>
                <p className="text-gray-300">
                  <span className="font-semibold text-red-400">Popularity:</span> {popularity}
                </p>
              </div>
              <p className="text-gray-300 col-span-full">
                <span className="font-semibold text-red-400">Production Companies:</span> {production_companies.join(", ")}
              </p>
            </div>
          </div>
        </div>
